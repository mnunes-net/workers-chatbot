...
export async function handleChatGet(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  const dbHistory: any = await env.chatbot.prepare(`SELECT role, content FROM history WHERE user_name = ?1 AND chat_id = ?2`)
        ...
        <div id="chat-container">
          ...
          <div id="chat-history">
            ${initialChatHistory
              .map(
                (entry) =>
                  `<p><strong>${entry.role === 'assistant' ? 'Chatbot' : 'You'}:</strong> ${entry.content.replace(/\\n/g, '<br/>')}</p>`
              )
              .join('')}
          </div>
          <form id="chat-form">
